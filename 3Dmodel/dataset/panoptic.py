...
class PanopticDataset(data.Dataset):
   ...
   def __getitem__(self, idx):
      ...
      EXTRA_BODIES = 7 - nj
      if EXTRA_BODIES == 7:
         joint = np.zeros((EXTRA_BODIES, 15, 2))
      elif EXTRA_BODIES != 0:
         pad = np.zeros((EXTRA_BODIES, 15, 2))
      ...
      if EXTRA_BODIES == 7:
         labels = np.zeros((EXTRA_BODIES, 15, 3))
      elif EXTRA_BODIES != 0:
         pad = np.zeros((EXTRA_BODIES, 15, 3))
      ...
      #print(np.amax(labels_heatmap), np.amin(labels_heatmap))
   ...
   def GenerateHeatmap(self, target):
      ...
      for i in range(t1*t2):
         heatmap_tmp = np.zeros((216, 384))
         x = int(target[i][0])
         y = int(target[i][1])
         ### Direct value
         if x < 384 and x >= 0 and y < 216 and y >= 0:
            heatmap_tmp[y][x] = target[i][2]
            heatmap_tmp = cv2.GaussianBlur(heatmap_tmp, (11, 11), 0)
            # am = np.amax(heatmap)
            # heatmap /= am / 255
