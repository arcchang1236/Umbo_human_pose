...
class Arc(nn.Module):
    ...
    def __init__(self, batch_size, output_size):
        ...
        self.upsample = Interpolate(self.output_size, mode='bilinear')
    ...
    def printShape(self, tmp):
        for i in range(len(tmp)):
            print("{0}: Shape = {1}".format(i+1, tmp[i]))
    ...
    def forward(self, x):
        ...
        #x = 1/(10*x+0.1)
        tmp.append(x.shape)
        #x = nn.functional.interpolate(x, size=(1080, 1920), mode='bilinear', align_corners=True)
        x = self.upsample(x)
        tmp.append(x.shape)
