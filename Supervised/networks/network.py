...
class CPN(nn.Module):
    def __init__(self, resnet, output_shape, num_class, pretrained=True):
        ...
        # 3D
        self.arc_net = Arc2(output_size=output_shape, in_channels=3, pretrained=True)

    def forward(self, x):
        # 2D
        res_out = self.resnet(x)
        #print("{} {} {} {}***".format(np.shape(res_out[0]), np.shape(res_out[1]), np.shape(res_out[2]), np.shape(res_out[3])))
        global_fms, global_outs = self.global_net(res_out)
        refine_out = self.refine_net(global_fms)
        # 3D
        depth_out = self.arc_net(res_out[0])
        #print(np.shape(res_out[0]), np.shape(global_fms[0]), np.shape(global_outs[0]), np.shape(refine_out[0]), np.shape(depth_out))
        return global_outs, refine_out, depth_out
